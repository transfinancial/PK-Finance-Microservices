"""
Excel Export Module (Unified)
==============================
Saves both mutual fund NAV and PSX stock data to formatted Excel files.
"""
...
# ══════════════════════════════════════════════════════════════════
#  MUFAP Mutual Funds Export
# ══════════════════════════════════════════════════════════════════

def save_to_excel(df: pd.DataFrame, filename: str = None) -> str:
    """Save mutual fund NAV DataFrame to a formatted Excel file."""
    ...
    ws["A2"] = f"Generated on: {now_utc5().strftime('%B %d, %Y at %I:%M %p')} (UTC+5)"
    ...
    wb.save(filepath)
    logger.info(f"MUFAP Excel saved: {filepath}")
    return filepath


# ══════════════════════════════════════════════════════════════════
#  PSX Stock Market Export
# ══════════════════════════════════════════════════════════════════
...
def save_stocks_to_excel(df: pd.DataFrame, filename: str = None) -> str:
    ...
    # Headers
    col_display = {
        "symbol": "Symbol", "sector_code": "Sector", "ldcp": "LDCP",
        "open": "Open", "high": "High", "low": "Low", "current": "Current",
        "change": "Change", "change_pct": "Change %", "volume": "Volume", "date": "Date",
    }
